...
class Employees_detail_model extends CI_Model {
    ...
    function get_stahistory($x) {
        $data = $this->db->query("SELECT d.* FROM status_history d LEFT JOIN employee e ON d.stahistory_employee_id = e.employee_id WHERE e.employee_id = '$x' ORDER BY stahistory_start ASC")->result_array();
    ...
    function add_education($data = array()) {
        ...
        if(isset($data['education_photo'])) {
            $this->db->set('education_photo', $data['education_photo']);
        }
    ...
    function add_stahistory($data = array()) {

        if (isset($data['stahistory_id'])) {
            $this->db->set('stahistoryt_id', $data['stahistory_id']);
        }

        if (isset($data['stahistory_start'])) {
            $this->db->set('stahistory_start', $data['stahistory_start']);
        }

        if (isset($data['stahistory_end'])) {
            $this->db->set('stahistory_end', $data['stahistory_end']);
        }

        if (isset($data['stahistory_name'])) {
            $this->db->set('stahistory_name', $data['stahistory_name']);
        }

        if (isset($data['stahistory_emp'])) {
            $this->db->set('stahistory_emp', $data['stahistory_emp']);
        }

        if (isset($data['stahistory_desc'])) {
            $this->db->set('stahistory_desc', $data['stahistory_desc']);
        }
        
        if (isset($data['stahistory_employee_id'])) {
            $this->db->set('stahistory_employee_id', $data['stahistory_employee_id']);
        }
        
        if (isset($data['stahistory_id'])) {
            $this->db->where('stahistory_id', $data['stahistory_id']);
            $this->db->update('status_history');
            $id = $data['stahistory_id'];
        } else {
    ...
    function delete_stahistory($id) {
        $this->db->where('stahistory_id', $id);
        $this->db->delete('status_history');
    }
}
