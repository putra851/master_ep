...
// Siswa
    ...
    $like   = str_replace(' ', '', 'GK'.$employee['majors_short_name'].$employee['employee_nip']);
    $tmp    = $this->Slip_model->get_noref($like, $majorsID);
    $data['noref'] = str_replace(' ', '', 'GK'.$employee['majors_short_name'].$employee['employee_nip'].$tmp);
  ...
  function add_slip(){
    if ($_POST == TRUE) {
			...
			$params['gaji_utama']        = $_POST['utama'];
			$params['gaji_status']       = $_POST['status'];
			$params['gaji_tunjangan']    = $_POST['tunjangan'];
			$params['gaji_induk']        = $_POST['induk'];
			$params['gaji_non_induk']    = $_POST['non_induk'];
			$params['gaji_standart']     = $_POST['standart'];
			$params['gaji_jam']          = $_POST['jam'];
            ...
            $paramssubsatu['subsatu_fungsional']= $this->input->post('premier_fungsional');
            $paramssubsatu['subsatu_tambahan']  = $this->input->post('premier_tambahan');
            $paramssubsatu['subsatu_gaji_id']   = $q['gaji_id'];
            
            $paramssubdua['subdua_transport']   = $this->input->post('sekunder_transport');
            $paramssubdua['subdua_hr']          = $this->input->post('sekunder_hr');
            $paramssubdua['subdua_penyesuaian'] = $this->input->post('sekunder_penyesuaian');
            $paramssubdua['subdua_piket']       = $this->input->post('sekunder_piket');
            $paramssubdua['subdua_kepala_keluarga'] = $this->input->post('sekunder_kepala_keluarga');
            $paramssubdua['subdua_kurleb']      = $this->input->post('sekunder_kurleb');
            $paramssubdua['subdua_koreksi']     = $this->input->post('sekunder_koreksi');
            $paramssubdua['subdua_gaji_id']     = $q['gaji_id'];
      
            $paramssubtiga['subtiga_absen']     = $this->input->post('potongan_absen');
            $paramssubtiga['subtiga_infak']     = $this->input->post('potongan_infak');
            $paramssubtiga['subtiga_bmt']       = $this->input->post('potongan_bmt');
            $paramssubtiga['subtiga_koperasi']  = $this->input->post('potongan_koperasi');
            $paramssubtiga['subtiga_qurban']    = $this->input->post('potongan_qurban');
            $paramssubtiga['subtiga_lain']      = $this->input->post('potongan_lain');
            $paramssubtiga['subtiga_spp_anak']  = $this->input->post('potongan_spp_anak');
            $paramssubtiga['subtiga_gaji_id']   = $q['gaji_id'];
      
            $this->Slip_model->set_subsatu($paramssubsatu);
            $this->Slip_model->set_subdua($paramssubdua);
  ...
  function print_slip($id){
     ob_start();
      ...
      $this->load->view('slip_print',$data);
        ...
        $pdf = new HTML2PDF('P',array('240', '100'),'en', false, 'ISO-8859-15',array(5, 5, 5, 10));
